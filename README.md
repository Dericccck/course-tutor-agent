...
- `qa` 场景会优先输出“先直接回答，再做 2-4 句补充解释”的短答案结构
- `summary` 场景会优先按固定三行结构输出：
  - 这节课主要讲什么
  - 它解决了什么问题
  - 关键收获是什么
- 最终返回的 `answer` 会在输出层统一去除首尾空白；如果模型返回空正文，会自动回退到简短兜底说明
- 最终返回的 `suggestions` 会在输出层统一做去重、去空、去问题原文，并限制最多 3 条
...
- 返回：
  ...
  - `suggestions`
  - `sources`
- 当前主流程会只把最关键的前几条证据 chunk 注入 prompt：
  - `qa` 默认前 3 条
  - `summary` 默认前 4 条
  - `study_plan` 默认前 5 条
- `sources` 不再强制回填全部检索结果，而是优先保留模型真实引用且合法的来源；如果模型返回非法来源，则退回到当前 prompt 中的关键证据来源
