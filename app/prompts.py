...
def build_user_prompt(question: str, retrieved_chunks: list[RetrievedChunk], memory: dict | None = None,) -> str:
    ...
    回答要求：
    1.优先基于前 3 条资料回答。
    2.先用 1 句话直接回答用户问题，再用 2-4 句补充解释。
    3.如果是概念题，优先给出“它是什么”，再说明“它为什么重要/和什么有关”。
    4.如果引用资料，请在 answer 中尽量显式提到资料编号，例如 [资料 1]。
    5.sources 字段只能填写上面“引用来源”中的真实值，并且只保留你实际使用过的 1-3 条。
    6.answer 不要写成长篇综述，也不要脱离资料做泛化扩展。

    请基于以上资料输出 JSON：
    {{
        "answer": "先直接回答问题，再做简短解释",
...
def build_summary_prompt(question: str, retrieved_chunks: list[RetrievedChunk], memory: dict | None = None) -> str:
    ...
    回答要求：
    1.优先基于前 4 条资料总结。
    2.回答尽量控制在 3 段以内，不要写成松散长文。
    3.如果引用资料，请在 answer 中尽量显式提到资料编号，例如 [资料 1]。
    4.sources 字段只能填写上面“引用来源”中的真实值，并且只保留你实际使用过的 1-3 条。

    请基于以上资料输出 JSON：
    {{
        "answer": "按固定结构总结这节课 / 这个 notebook",
        "suggestions": ["给出1-3条后续学习建议"],
        "sources": ["列出你实际使用的资料来源路径"]
    }}

    answer 请直接按下面 3 行结构输出，不要把三点合并成一整段：
    1. 这节课主要讲什么：...
    2. 它解决了什么问题：...
    3. 关键收获是什么：...
...
def build_study_plan_prompt(question: str, retrieved_chunks: list[RetrievedChunk], memory: dict | None = None,) -> str:
    ...
    - 如果用户已经完成了一部分主题，回答时应明确说明“基于已完成内容，下一步建议是什么”
    - 当存在已完成主题时，优先把回答写成“你已经完成了 A，因此建议继续学习 B、C”这种形式
    - 如果用户设置了学习范围，应优先在该范围内安排学习路线
    - 如果某些推荐内容超出用户当前学习范围，应明确标注这是范围外的补充建议，而不是当前主路线
    """